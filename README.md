...
#### V2 해결점:
- 읽기 작업에서 속도를 개선하기 위해 Redis 캐시를 도입하여 데이터베이스 접근을 최소화했습니다.
...
<details>
  ...
  - Redis 캐시를 통해 읽기 작업의 성능을 개선하고 데이터베이스 접근을 최소화.
  - **write-back** 방식을 통해 자주 발생하는 쓰기 작업의 효율성을 극대화.
  - **write-through** 방식을 사용해 드물게 발생하는 쓰기 작업에서 데이터의 일관성과 성능을 모두 확보.
...
<details>
  ...
  - **Sorted Set**을 이용해 검색 기록을 빠르고 효율적으로 저장하고 조회할 수 있도록 최적화.
  - **Set**을 통해 팔로우/팔로잉 유저 데이터를 효과적으로 관리하여 빠른 데이터 접근 및 성능 향상을 달성했습니다.
...
#### V1 문제점:
- 게시글 작성 및 수정 시 데이터베이스에 과도한 부하가 발생하였고, 조회 시 속도가 느려지는 문제가 있었습니다.
- 실시간 작업(댓글, 좋아요 등)이 서버 간 동기화 문제로 지연되는 경우가 발생했습니다.

#### V2 해결점:
- Redis 캐시를 사용하여 데이터베이스 접근을 최소화하고, 게시글 데이터의 빠른 접근을 가능하게 하였습니다.
- Redis Pub/Sub를 사용해 분산 서버 간의 실시간 작업(댓글, 좋아요 등)을 효율적으로 처리하여 성능을 개선했습니다.

<details>
  <summary>🔧 성능 개선 보기</summary>
  
  - 게시글 데이터의 빠른 접근을 위해 Redis 캐시를 사용하여 데이터베이스 부하를 줄임.
  - 분산 서버 환경에서 댓글, 좋아요 등 실시간 작업을 Redis Pub/Sub로 처리하여 성능을 개선.
  
</details>


  
  - Redis 캐시를 사용하여 검색 시 데이터베이스 접근을 최소화하고, 자동완성 기능의 속도 향상.
  </details>
